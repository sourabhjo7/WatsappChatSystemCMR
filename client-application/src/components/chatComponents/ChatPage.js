...
function ChatPage({ socket, userData, setIsLogedin }) {
    ...
    await setActiveAgents(() => {
      return activeAgents.filter((agent) => {
        return (
          ...
          agent.creatorUID === userData.creatorUID
        );
      });
    });
  };
  ...
  const disconnect = async (room) => {
    await socket.emit("disconnect_chat", {
      chat: currActiveChat,
  ...
  const reassign = async (e, action, room) => {
    ...
    if (action === "reassign") {
      ...
      //if action is escalation
      managerID = userData.creatorUID;

      //Adding ecalation to DB
      await newescalation(
        room,
        currActiveChat.phoneNo,
        userData.name,
        managerID
      );
  ...
  const ReassignCom = ({ room }) => {
    return (
      <div>
        <select className="agentSelect" id="agentSelect">
          {activeAgents.map((agent, index) => {
            return (
              <option key={index} value={agent.email}>
                {agent.name}
              </option>
            );
          })}
  ...
  useEffect(() => {
    ...
    socket.on("room_joined", (data) => {
      ...
      setCurrJoinedChats((curr) => {
        return [
          ...curr,
          { room: data.room, phoneNo: data.phoneNo, messageList },
        ];
      });
  ...
  return (
    <div className="rootCon" id="rootCon">
      <Sidebar
        role={process.env.REACT_APP_AgentRole}
